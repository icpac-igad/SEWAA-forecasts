...
CLASSES = 4  # run08/run02: 4 rain bins (quartiles). = len(training_weights). (run06 used 5)
...
def write_data(year,
    ...
    # binning: quartiles of the 128x128 chunk-mean of raw mm/hr, derived from the
    # CHIRPS 2018 distribution by compute_chirps_bins.py (25.1/25.1/25.0/24.7 split).
    # Do NOT reuse the RFE2 values [0.0059, 0.0362, 0.0761] -- CHIRPS is land-only
    # and its wet tail sits higher, so the cut points genuinely differ.
    # run06: run02 quartiles + 95th-pct extreme cut (0.2813 mm/hr ~= 6.8 mm/day
    # patch-mean), derived by compute_chirps_bins_run06.py -> class 4 = top 5% of patches.
    bins = [0.0082, 0.0439, 0.1234, 0.2813]
    assert num_class == 5
    ...
    for time_idx in range(1,2):
        ...
        for fh in range(num_class):
            flename = os.path.join(folder, f"{year}_{LEAD_IDX}.{fh}.tfrecords")
        ...
        for batch in range(len(dgc)):
            ...
            for ii in range(nsamples):
                ...
                if truth_mean < bins[0]:
                    ...
                    clss = 2
                elif truth_mean < bins[3]:
                    clss = 3
                else:
                    clss = 4   # EXTREME (top 5% of patch-means)
