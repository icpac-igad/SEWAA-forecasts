...
def load_truth_and_mask(date,
    ...
    print("WARNING: LEAD_IDX = {LEAD_IDX} used in data.py")
    valid_dt = fcst_date + datetime.timedelta(hours=int(LEAD_IDX)*HOURS)  # All forecasts start at 06:00 the next day
    fname = valid_dt.strftime('%Y%m%d_%H')
    data_path = os.path.join(TRUTH_PATH, f"{valid_dt.year}/{fname}.nc")
