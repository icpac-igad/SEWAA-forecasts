#!/usr/bin/env python
"""Run the operational cGAN-RFE (run13) 24h forecast for one date.
...
CONFIG = "forecast_run13_operational.yaml"
...
def parse_args():
    ...
    p.add_argument("--accumulation", default="24h",
                   help="Accumulation window. Only 24h is deployed for run13.")
    ...
    if a.accumulation not in ("24h", "24"):
        sys.exit("ERROR: this deployment runs 24h accumulations only (RFE2 run13 is a 24h model).")
...
def run_forecast(date_str):
    ...
    env = os.environ.copy()
    env["CGAN_FIELD_SET"] = "run11"      # run13 reuses run11's field set (13 IFS + clim mean/sd = 28ch)
    env["PYTHONPATH"] = dsr
    ld = cuda_ld_path()
    if ld:
        env["LD_LIBRARY_PATH"] = ld + ":" + env.get("LD_LIBRARY_PATH", "")
    print(f"Running run13 24h forecast + counts for {date_str}")
...
def main():
    ...
    print(f"Producing 24h cGAN-RFE (run13) forecast initialised {date_str} 0000Z.")
    ...
    if delete_forecasts:
        ...
        raw = os.path.join(ROOT, "24h_accumulations", "cGAN",
                           "fcst_run13_operational", f"GAN_fcst_crps_{date_str}_00Z.nc")
