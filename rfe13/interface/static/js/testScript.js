...
let modelName = "24h accumulation"
// Training datasets: "chirps", "rfe"
let trainingDataset = "chirps";
...
// Returns the data URL prefix for the current training dataset.
// In combined mode (data-combined="true" on <body>), data is served under
// /chirps-data/ and /rfe-data/. In standalone mode, data is at /data/.
function getDataPrefix() {
	if (document.body.dataset.combined === "true") {
		if (trainingDataset == "rfe") {
			return "/rfe-data";
		}
		return "/chirps-data";
	}
...
// Model description lookup keyed by training dataset and accumulation
const modelDescriptions = {
	"chirps": {
		"6h accumulation": ': The <a href="https://www.ecmwf.int/" target="_blank">ECMWF</a> <a href="https://confluence.ecmwf.int/display/FUG/Section+2+The+ECMWF+Integrated+Forecasting+System+-+IFS" target="_blank">IFS</a> output is post-processed using <a href="https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2022MS003120" target="_blank">cGAN</a> trained on <a href="https://www.chc.ucsb.edu/data/chirps" target="_blank">CHIRPS</a> from 2018 to 2020 to produce forecasts of 6h rainfall intervals. Model: run07.',
		"24h accumulation": ': The <a href="https://www.ecmwf.int/" target="_blank">ECMWF</a> <a href="https://confluence.ecmwf.int/display/FUG/Section+2+The+ECMWF+Integrated+Forecasting+System+-+IFS" target="_blank">IFS</a> output is post-processed using <a href="https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2022MS003120" target="_blank">cGAN</a> trained on <a href="https://www.chc.ucsb.edu/data/chirps" target="_blank">CHIRPS</a> from 2018 to 2020 to produce forecasts of 24h rainfall intervals. Model: run07 (extremes champion, no cape).'
	},
	"rfe": {
		"6h accumulation": ': The <a href="https://www.ecmwf.int/" target="_blank">ECMWF</a> <a href="https://confluence.ecmwf.int/display/FUG/Section+2+The+ECMWF+Integrated+Forecasting+System+-+IFS" target="_blank">IFS</a> output is post-processed using <a href="https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2022MS003120" target="_blank">cGAN</a> trained on <a href="https://www.cpc.ncep.noaa.gov/products/fews/rfe.shtml" target="_blank">RFE2</a> from 2018 to 2021 to produce forecasts of 6h rainfall intervals. Model version 3 (run13).',
		"24h accumulation": ': The <a href="https://www.ecmwf.int/" target="_blank">ECMWF</a> <a href="https://confluence.ecmwf.int/display/FUG/Section+2+The+ECMWF+Integrated+Forecasting+System+-+IFS" target="_blank">IFS</a> output is post-processed using <a href="https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2022MS003120" target="_blank">cGAN</a> trained on <a href="https://www.cpc.ncep.noaa.gov/products/fews/rfe.shtml" target="_blank">RFE2</a> from 2018 to 2021 to produce forecasts of 24h rainfall intervals. Model version 3 (run13 &mdash; extremes-tuned).'
	}
};

// Update the model info text based on current training dataset and model
function updateModelInfo() {
	let desc = modelDescriptions[trainingDataset][modelName] || "";
	document.getElementById("modelInfo").innerHTML = desc;
}

// Called by the trainingDatasetSelect menu
async function trainingDatasetSelect() {
	trainingDataset = document.getElementById("trainingDatasetSelect").value;
	updateModelInfo();
	await loadDates();
	await loadForecast();
	drawMarker = false;
	document.getElementById("removeHistBttn").style.display = "none";
	drawPlots();
}
...
function regionSelect() {
	...
	drawMarker = false;	// No longer draw the histograms
	document.getElementById("removeHistBttn").style.display = "none";	// Hide the button
	window.adminBBox = null;		// clear any admin-1 zoom when the country changes
	window.adminRegionName = null;		// clear single-region isolation
	populateAdmin1(regionName);		// fill/show the Admin 1 sub-selector for this country
	drawPlots();
}

// Populate the Admin 1 sub-selector for the chosen country (hidden for the East Africa view).
async function populateAdmin1(country) {
	const grp = document.getElementById("admin1Group");
	const sel = document.getElementById("admin1Select");
	if (country == "East Africa" || country == "ICPAC" || country == "all") {
		grp.style.display = "none";
		sel.innerHTML = '<option value="">— whole country —</option>';
		return;
	}
	if (!window.regionBBoxes) {
		try { window.regionBBoxes = await (await fetch("/static/boundaries/region_bboxes.json")).json(); }
		catch (e) { window.regionBBoxes = {}; }
	}
	const regs = window.regionBBoxes[country] || [];
	let html = '<option value="">— whole country —</option>';
	for (const r of regs) html += '<option value="' + r.name + '">' + r.name + '</option>';
	sel.innerHTML = html;
	grp.style.display = (regs.length > 1) ? "" : "none";	// no sub-selector if only one region
}

// Zoom the map to a selected admin-1 region (or back to the whole country when blank).
function admin1Select() {
	const country = document.getElementById("regionSelect").value;
	const name = document.getElementById("admin1Select").value;
	window.adminBBox = null;
	window.adminRegionName = name || null;	// isolate this single region's outline (null = whole country)
	if (name && window.regionBBoxes && window.regionBBoxes[country]) {
		const hit = window.regionBBoxes[country].find(r => r.name === name);
		if (hit) window.adminBBox = hit.bbox;
	}
	drawMarker = false;
	document.getElementById("removeHistBttn").style.display = "none";
	drawPlots();
...
async function loadForecast() {
	...
	// Data path prefix depends on selected training dataset
	let dataPrefix = getDataPrefix();

	// If we should load all valid times
	if (validTimeMenu == "All") {
		for (let i=0;i<validTimes.length;i++) {
			// The cGAN forecast file to load
			let fileName = dataPrefix+"/"+countsDir+"/"+year+"/counts_"+year
										 +month.padStart(2,'0')
										 +day.padStart(2,'0')
										 +"_"+time.padStart(2,'0')
										 +"_"+validTimes[i]+"h.nc";
...
async function loadDates() {
	...
	let fileName;
	if (modelName == "6h accumulation") {
		fileName = dataPrefix+"/counts_6h/available_dates.json?"+dateLoadNumber;
	} else if (modelName == "24h accumulation") {
		fileName = dataPrefix+"/counts_24h/available_dates.json?"+dateLoadNumber;
	}
...
function initControls() {
	...
	document.getElementById("trainingDatasetSelect").value = trainingDataset;
