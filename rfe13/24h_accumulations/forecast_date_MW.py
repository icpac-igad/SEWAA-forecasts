...
if field_set == "14field":
    ...
    raise ValueError(f"MODEL.field_set must be 14field/13field/4field/run11, got {field_set!r}")
    ...
    for valid_time_num in range(len(valid_times_forecast)):
        ...
        if clim_channels:
            ...
            network_fcst_input = np.concatenate(
                [network_fcst_input, climatology_channel(d.strftime('%Y%m%d'), lead_days=valid_time_num)], axis=-1)
