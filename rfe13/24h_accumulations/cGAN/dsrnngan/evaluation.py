...
def eval_one_chkpt(*,
    ...
    data_gen_iter = iter(data_gen)
    tpidx = 2*data.all_fcst_fields.index('tp')  # 4*idx has tp ens mean
...
def evaluate_multiple_checkpoints(*,
    ...
    # Per-image (per-day) CRPS, so the model can be compared to a baseline with a PAIRED test.
    # eval_validation.txt keeps only the mean over images, which throws away the pairing and
    # leaves the unpaired day-to-day spread (SE ~0.0056 on 25 days) as the apparent uncertainty.
    perday_fname = os.path.splitext(log_fname)[0] + "_perday.txt"
    log_line(perday_fname, f"# num_images {num_images}  ensemble_size {ensemble_size}  seed {data_gen_valid.seed}")
    log_line(perday_fname, "# dates " + " ".join(str(d) for d in data_gen_valid.dates[:num_images]))
    log_line(perday_fname, "# time_idxs " + " ".join(str(t) for t in data_gen_valid.time_idxs[:num_images]))
    log_line(perday_fname, "# columns: N then one no_pooling CRPS per image, in the order above")
    ...
    for model_number in model_numbers:
        ...
        log_line(perday_fname,
                 f"{model_number} " + " ".join(f"{v:.6f}" for v in np.asarray(crps['no_pooling'])))
