...
from data import HOURS, all_fcst_fields, accumulated_fields, nonnegative_fields, fcst_norm, logprec, denormalise, load_hires_constants, sanitise_fcst_input
import read_config
...
if (True): 
    ...
    # Guard against corrupt IFS fields (e.g. an all-NaN u700/v700 slice, as in 2024-01-28).
    # A NaN channel would otherwise poison the whole forecast. sanitise fills isolated NaNs
    # with the normalised field mean (0) so the good fields still produce a forecast; if a
    # critical field is dead the date is unusable and we write no garbage forecast.
    _date_str = f"{d.year}{d.month:02d}{d.day:02d}"
    network_fcst_input, _usable, _rep = sanitise_fcst_input(
        network_fcst_input, all_fcst_fields, _date_str)
    if _rep["dead_fields"]:
        print(f"WARNING: corrupt IFS input on {_date_str}: {_rep['dead_fields']}", flush=True)

    if not _usable:
        # critical field missing -> do not emit a NaN/garbage forecast; clean up and exit
        # cleanly so the per-date caller (run_forecast.py) moves on to the next date.
        print(f"SKIPPING forecast for {_date_str}: critical field(s) missing/NaN "
              f"({_rep['dead_fields']}). No forecast written.", flush=True)
