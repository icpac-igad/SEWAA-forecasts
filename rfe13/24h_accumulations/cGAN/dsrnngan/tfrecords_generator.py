...
import read_config
from data import all_fcst_fields, denormalise, get_dates, HOURS, USE_CLIMATOLOGY, CLIM_CHANNELS
...
DEFAULT_FCST_SHAPE = (128, 128, 2*len(all_fcst_fields) + (CLIM_CHANNELS if USE_CLIMATOLOGY else 0))
...
def write_data(year,
    ...
    # binning: quartiles from RFE2 2018 distribution (mm/hr after /24 conversion)
    bins = [0.0059, 0.0362, 0.0761]
    assert num_class == 4
    ...
    for time_idx in range(1,2):
        ...
        for fh in range(num_class):
            ...
            # compress generated TFRecords, courtesy Oxford's ARC
