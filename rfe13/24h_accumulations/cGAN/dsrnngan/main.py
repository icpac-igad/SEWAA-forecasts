...
if __name__ == "__main__":
    ...
    parser.add_argument('--overwrite', dest='overwrite', action='store_true',
                        help="Allow training from scratch into a log folder that already holds "
                             "a run, destroying its weights and log.txt")
    ...
    # Mirror of the check above. '--eval_*' only selects HOW MANY checkpoints to score; on its
    # own it does not request evaluation and does not disable training.
    if args.evalnum is not None and not args.evaluate:
        raise RuntimeError(
            f"You passed '--eval_{args.evalnum}' but not '--evaluate', so no evaluation would "
            "run. '--eval_*' only sets the number of checkpoints to score; training stays ON "
            "unless you pass '--no_train'. To evaluate an existing run, use:\n"
            "    --no_train --evaluate --eval_full")
    ...
    # Training without --restart starts at sample 0 and overwrites any run already in this
    # folder (weights, log.txt, run_status.json). run11 was destroyed this way on 2026-07-22.
    if args.do_training and not args.restart and not args.overwrite:
        existing = sorted(Path(model_weights_root).glob("gen_weights-*.h5"))
        if existing or os.path.exists(os.path.join(log_folder, "run_status.json")):
            raise RuntimeError(
                f"'{log_folder}' already contains a run ({len(existing)} saved checkpoints). "
                "Training without '--restart' would start from sample 0 and overwrite its "
                "weights and log.txt.\n"
                "    to resume it             : add --restart\n"
                "    to evaluate it           : add --no_train --evaluate --eval_full\n"
                "    to deliberately redo it  : add --overwrite  (DESTRUCTIVE)")
    ...
    if problem_type == "normal":
        ...
        if getattr(data, "USE_CLIMATOLOGY", False):
            # + RFE climatology mean & sd (run11); CLIM_CHANNELS lives in data.py
    ...
    if args.do_training:
        # initialize experiment tracking (Trackio): logs losses per checkpoint
        # plus automatic GPU/CPU utilization in the background
        run_name = os.path.basename(os.path.normpath(log_folder))
        trackio.init(
            project="sewaa-cgan-rfe2",
            name=run_name,
            config=setup_params,
            resume="allow" if args.restart else "never",
            auto_log_gpu=True,
            auto_log_cpu=True,
        )
        ...
        while training_samples < num_samples:  # main training loop
            ...
            trackio.log(loss_log, step=training_samples)
    ...
    # optional: restrict to checkpoints >= EVAL_FROM (resume/partial eval).
    # Inert unless the EVAL_FROM env var is set. Lets you finish an interrupted
    # eval by scoring only the not-yet-done tail (appends to eval_validation.txt).
    _eval_from = os.environ.get("EVAL_FROM")
    if _eval_from:
        model_numbers = [m for m in model_numbers if m >= int(_eval_from)]
        print(f"EVAL_FROM={_eval_from}: evaluating {len(model_numbers)} checkpoints "
              f"(from {model_numbers[0] if model_numbers else 'none'})")
